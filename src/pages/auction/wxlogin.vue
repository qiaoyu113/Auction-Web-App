...
//                    console.log('code:',code);
                          commonService.getWXinfo({code: code,state:'wx',platform:'WXH5'}).then(function (res) {
                              if(res.data.code==210100){
                                 window.localStorage.setItem('token',res.data.datas)
                                 that.$router.replace({name:'home'})

                              }else if(res.data.code==210101){
                                window.localStorage.setItem('redisInfoKey',res.data.datas.redisInfoKey)
                                window.localStorage.setItem('headImg',res.data.datas.headImg)
                                 that.$router.replace({name:'login',query:{wx:'wx'}})
                              }
                          })
